...
const handleDateClick = (day: { day: number; currentMonth: boolean; date: Date }) => {
  ...
  if (day.date < minDate) {
    ...
    setTimeout(() => { warningMessage.value = '' }, 3000)
    return
  }
...
const handleSubmit = async () => {
  ...
  try {
    await leaveApi.createKaryawan({
      jenis_cuti: form.value.jenis_cuti,
...
<template>
  <div>
    <p class="text-sm text-gray-500 mb-6">
      Silakan lengkapi formulir di bawah ini untuk mengajukan permohonan cuti Anda.<br />
    ...
    <div v-else class="flex gap-6">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100">
          ...
          <div class="p-6 border-b border-gray-100 bg-blue-50">
            <div class="flex items-center justify-between mb-4">
              ...
              <div class="flex items-center gap-4">
                <span v-if="selectedDaysCount > 0" class="text-sm text-blue-600 font-medium">
            ...
            <div class="flex items-center justify-between mb-4">
              ...
              <h4 class="text-xl font-bold text-gray-800">{{ monthNames[currentMonth] }} {{ currentYear }}</h4>
              <button
                @click="nextMonth"
                class="p-2 hover:bg-gray-100 rounded-lg transition-colors cursor-pointer"
              >
              ...
              <div class="grid grid-cols-7">
                <div
                  ...
                  :class="[
                    ...
                    day.currentMonth && isCutiBersama(day.date) && !isDateSelected(day.date) && 'bg-red-50',
                    day.currentMonth && isHoliday(day.date) && !isCutiBersama(day.date) && !isDateSelected(day.date) && 'bg-red-50',
                    day.currentMonth && !isPastDate(day.date) && !isDateSelected(day.date) && !isHoliday(day.date) && 'hover:bg-blue-100 cursor-pointer',
                    isDateSelected(day.date) && 'bg-blue-900 text-white font-medium'
                  ]"
                >
                  <span :class="[
                    'inline-block w-7 h-7 leading-7 text-center rounded-full text-sm',
                    isDateSelected(day.date) && 'bg-blue-900 text-white',
                    !isDateSelected(day.date) && day.currentMonth && (index % 7 === 0 || index % 7 === 6) && 'text-red-400'
                  ]">
                    {{ day.day }}
                  </span>
                  <div
                    v-if="isHoliday(day.date) && day.currentMonth && !isDateSelected(day.date)"
            ...
            <div class="flex items-center gap-4 text-xs text-gray-500 mt-4">
              <div class="flex items-center gap-2">
                <div class="w-3 h-3 bg-red-50 border border-red-300 rounded-full"></div>
            ...
            <div v-if="warningMessage" class="mt-4 p-3 bg-red-50 border border-red-300 rounded-lg text-sm text-red-600 font-medium">
              {{ warningMessage }}
            </div>
          </div>
          ...
          <div class="p-6 border-b border-gray-100 space-y-4">
            <div class="grid grid-cols-2 gap-4">
              <div>
                ...
                >
                  <option v-for="option in jenisCutiOptions" :key="option" :value="option">
                    {{ option }}
                  </option>
                </select>
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Delegasi Tugas (Backup)</label>
                <input
                  v-model="form.pengganti"
                  type="number"
          ...
          <div class="p-6">
            <div class="bg-gray-50 rounded-lg p-4 mb-4">
              <h4 class="font-semibold text-gray-800 mb-2">Pernyataan Pengajuan Cuti</h4>
              <p class="text-sm text-gray-600 mb-3">
                Saya mengajukan cuti pada tanggal <span class="font-medium">{{ form.tanggal_mulai || '-' }}</span> s.d. <span class="font-medium">{{ form.tanggal_selesai || '-' }}</span> (<span class="font-medium">{{ selectedDaysCount }}</span> hari), dengan alasan: <span class="font-medium">{{ form.keterangan || '-' }}</span>.
              </p>
              <p class="text-sm text-gray-600 mb-2">Dengan ini saya menyatakan bahwa:</p>
              <ul class="text-sm text-gray-600 space-y-1 ml-4 list-disc">
                <li>Pengajuan cuti ini telah saya diskusikan dengan tim dan disetujui oleh anggota tim terkait.</li>
                <li>Pekerjaan/tanggung jawab saya selama masa cuti telah memiliki personel pengganti (backup).</li>
                <li>Saya bertanggung jawab penuh atas kebenaran informasi yang saya sampaikan di atas.</li>
              </ul>
            </div>
      ...
      <div class="w-80 space-y-6">
        ...
        <div class="bg-blue-50 rounded-xl p-6">
          ...
          <ul class="space-y-4">
            <li class="flex items-start gap-3">
              ...
              <span class="text-sm text-gray-700">Maksimal <strong>4 hari</strong> berturut-turut untuk cuti reguler.</span>
            </li>
            <li class="flex items-start gap-3">
              <span class="w-5 h-5 bg-blue-600 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5">
                <svg class="w-3 h-3 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <span class="text-sm text-gray-700">Pengajuan minimal dilakukan <strong>H-3</strong> sebelum tanggal cuti.</span>
            </li>
            <li class="flex items-start gap-3">
              <span class="w-5 h-5 bg-blue-600 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5">
                <svg class="w-3 h-3 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <span class="text-sm text-gray-700">Saldo cuti tahunan Anda harus mencukupi jumlah hari yang diajukan.</span>
            </li>
            <li class="flex items-start gap-3">
              <span class="w-5 h-5 bg-blue-600 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5">
                <svg class="w-3 h-3 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <span class="text-sm text-gray-700">Pastikan tidak ada <strong>konflik jadwal</strong> penting di departemen Anda.</span>
            </li>
          </ul>
        </div>
        ...
        <div class="bg-gray-600 rounded-xl shadow-sm p-6 text-white">
          ...
          <p class="text-4xl font-bold mb-4">{{ user?.sisa_cuti || 0 }} <span class="text-lg font-normal text-gray-200">Hari</span></p>
          <div class="w-full bg-gray-400 rounded-full h-2 mb-3">
            <div class="bg-white rounded-full h-2" :style="{ width: (((user?.total_cuti || 0) - (user?.sisa_cuti || 0)) / (user?.total_cuti || 1)) * 100 + '%' }"></div>
          </div>
          <div class="flex justify-between text-xs text-gray-200">
            <span>Digunakan: {{ (user?.total_cuti || 0) - (user?.sisa_cuti || 0) }}</span>
