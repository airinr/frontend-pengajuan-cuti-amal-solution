  ...
  try {
    ...
    if (
      ...
      cutiMendatangList.value = mendatangRes.value.data.map((item: any) => ({
        ...
        waktu_teks: item.tanggal
          ? formatDateRange(item.tanggal, item.tanggal_selesai || item.tanggal)
          : "Mendatang",
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-5">
        ...
        >
          <div>
            ...
            <p class="text-[11px] font-semibold text-gray-400 uppercase tracking-wider mt-4">CUTI BULAN INI</p>
          </div>
          <p class="text-3xl lg:text-4xl font-extrabold text-gray-900 mt-1 tracking-tight">{{ stats.total_cuti_bulan_ini }}</p>
          <div class="absolute -right-4 -bottom-4 w-20 h-20 bg-blue-50/40 rounded-full blur-xl pointer-events-none"></div>
        </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-5 lg:gap-6 items-start">
          ...
          <div>
            ...
            <div v-else class="pt-4 space-y-3">
              <div v-for="(item, index) in cutiMendatangList" :key="index" class="flex items-center justify-between py-2">
                <div>
                  <p class="text-sm font-bold text-gray-900 leading-snug">{{ item.nama }}</p>
                  <p class="text-xs text-gray-400 font-medium mt-0.5">{{ item.jenis_cuti }} • {{ item.departemen }}</p>
                </div>
                <div class="flex items-center gap-3 shrink-0">
                  <span class="text-xs font-medium text-gray-500">{{ item.waktu_teks }}</span>
                  <span class="inline-block text-[11px] px-2.5 py-0.5 rounded-full font-semibold border bg-blue-50 text-blue-600 border-blue-100">{{ item.status }}</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="lg:col-span-5 bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex flex-col">
          ...
          <div v-else class="relative space-y-6 pl-1">
            ...
            <div v-for="(activity, index) in activities.slice(0, 5)" :key="index" class="relative flex items-start gap-3.5 z-10">
              <div
                :class="[
              ...
              >
                <svg v-if="activity.jenis_aktivitas === 'approve'" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7" />
                </svg>
                <svg v-else-if="activity.jenis_aktivitas === 'reject'" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <svg v-else-if="activity.jenis_aktivitas === 'submit'" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div class="flex-1 min-w-0 pt-0.5">
                <p class="text-sm font-semibold text-gray-800 leading-snug">{{ activity.keterangan }}</p>
                <p class="text-xs text-gray-400 mt-1 font-medium">{{ activity.tanggal }}</p>
              </div>
            </div>
          </div>
        </div>
