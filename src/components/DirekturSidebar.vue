...
const direkturMenu = [
  { label: "Dashboard", icon: "grid", route: "/direktur/dashboard" },
  { label: "Persetujuan", icon: "check-circle", route: "/direktur/persetujuan" },
  { label: "Log & Rekap Cuti", icon: "clipboard", route: "/direktur/log-rekap-cuti" },
...
const personalMenu = [
  { label: "Profil", icon: "user", route: "/direktur/profil" },
];
  ...
  <div>
    ...
    >
      ...
      <nav class="flex-1 p-4 overflow-y-auto">
        <!-- Direktur Admin -->
        <p class="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-2 px-4">Direktur Admin</p>
        ...
        <ul class="space-y-1">
          <li v-for="item in personalMenu" :key="item.route">
            ...
            >
              <svg v-if="item.icon === 'user'" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
              </svg>
              {{ item.label }}
            </button>
